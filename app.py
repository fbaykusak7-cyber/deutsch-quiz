...
app = Flask(__name__)
app.secret_key = "ultimate_wrong_mode"
...
def quiz():
    ...
    if request.method == "POST":
        ...
        if selected:
            ...
            if selected == correct:
                ...
                # WRONG MODE içindeysek doğru yapılanı sil
                if level == "wrong":
                    session["wrong_words"] = [
                ...
                # yanlış listeye ekle (tekrar eklenmesin)
                if not any(w["de"] == german for w in session["wrong_words"]):
                    session["wrong_words"].append({
                        "de": german,
                        "en": correct
                    })
    ...
    pool = get_pool(level)
    if not pool:
        return "Wrong list empty. Good job!"
